...
this.Frd.status='noo';
          ...
          this.ser.getUserbyId(this.id).subscribe(d=>{
                ...
                this.ser.getFollowers(this.id).subscribe(d=>{
                  this.followers=d;
                  console.log("folowe:"+d)
                  this.ser.getFollowers(this.loggedInid).subscribe(d=>{
                    this.loggedinuserf=d;
                    console.log("loggeduserfolowe:"+this.loggedinuserf)
                    for(let i=0;i<this.followers.length;i++){
                      for(let j=0;j<this.loggedinuserf.length;j++){
                                       if(JSON.stringify(this.followers[i].id)==JSON.stringify(this.loggedinuserf[j].id)){
                  ...
                  this.ser.getFollowing(this.loggedInid).subscribe(dd=>{
                    ...
                    for(let i=0;i<this.following.length;i++){
                      for(let j=0;j<this.loggedinFolow.length;j++){
                        if(JSON.stringify(this.following[i].id)==JSON.stringify(this.loggedinFolow[j].id)){
